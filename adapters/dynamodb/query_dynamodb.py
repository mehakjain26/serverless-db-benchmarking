...
TABLE_NAME = os.environ.get("DYNAMO_TABLE", "gtfs")
...
def next_departures(transport_id, stop_id, after_time):
    ...
    # Collect unique trip_ids and look up trips directly (much faster than FilterExpression)
    trip_ids = list({st["trip_id"] for st in stop_times})
    trips = {}
    for tid in trip_ids:
        resp_t = table.get_item(Key={"pk": f"{transport_id}#trips", "sk": tid})
        if resp_t.get("Item"):
